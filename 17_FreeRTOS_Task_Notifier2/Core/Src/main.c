...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	if(GPIO_Pin == GPIO_PIN_13)
	{
		...
		/* check if higher priority task woken, and request a context switch if necessary */
...
{/* USER CODE BEGIN 5 */
	  ...
	  for(;;)
	  {
	    ...
	    if(xTaskNotifyWait(3, 0, &notificationValue, portMAX_DELAY) == pdTRUE)
	    {
	      if(notificationValue >= 1) // Changed condition to check any non-zero notification
	      {
	    	HAL_GPIO_WritePin(BSP_LED_GPIO_Port, BSP_LED_Pin, GPIO_PIN_SET);
	        printf("Button pressed, LED on for 2 seconds\r\n"); // Debug message
	        osDelay(2000);
	        HAL_GPIO_WritePin(BSP_LED_GPIO_Port, BSP_LED_Pin, GPIO_PIN_RESET);
	      }
	    }
