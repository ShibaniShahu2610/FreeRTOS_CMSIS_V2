...
/* Definitions for Switch_Task */
osThreadId_t Switch_TaskHandle;
const osThreadAttr_t Switch_Task_attributes = {
  .name = "Switch_Task",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
...
void Start_Switch_Task(void *argument);
...
int main(void)
{
  ...
  /* creation of Switch_Task */
  Switch_TaskHandle = osThreadNew(Start_Switch_Task, NULL, &Switch_Task_attributes);
...
/* USER CODE BEGIN Header_Start_Switch_Task */
/**
* @brief Function implementing the Switch_Task thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_Start_Switch_Task */
void Start_Switch_Task(void *argument)
{
  /* USER CODE BEGIN Start_Switch_Task */
  /* Infinite loop */
  for(;;)
  {
	if(HAL_GPIO_ReadPin(BSP_BUTTON_GPIO_Port, BSP_BUTTON_Pin) == GPIO_PIN_SET )
	{
		/* Button pressed send a Task notification to LED Task */
		xTaskNotifyGive(LED_TaskHandle);
	}
	osDelay(100);
  }
  /* USER CODE END Start_Switch_Task */
}
